...
Ограничения:
- при подаче на вход нулевого указателя на массив (`arr`) или неположительного значения длины (`length <= 0`), в поток вывода (`os`) должно вывестить сообщение об ошибке `
- 9` или `"Invalid argument: length\n"`, соответственно
